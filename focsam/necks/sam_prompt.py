...
class SAMPromptEncoder(nn.Module):
    ...
    def forward(self, image_embeds, points, boxes=None, prev_logits=None):
        ...
        sparse_embeds = torch.empty((B, 0, self.embed_dim), device=self.device)
        if points is not None:
            coords, labels = points
            point_embeds = self.embed_points(
                input_image_size, coords, labels, pad=(boxes is None))
            sparse_embeds = torch.cat([sparse_embeds, point_embeds], dim=1)
        if boxes is not None:
            box_embeds = self.embed_boxes(input_image_size, boxes)
        ...
        if tuple(dense_embeds.shape[-2:]) != image_embed_size:
            dense_embeds = F.interpolate(
                dense_embeds, image_embed_size, mode='bilinear')
        image_pos_embeds = self.image_pos_embeds
        if tuple(image_pos_embeds.shape[-2:]) != image_embed_size:
            image_pos_embeds = F.interpolate(
                image_pos_embeds, image_embed_size, mode='bilinear')
        return dict(ori_image_embeds=ori_image_embeds,
